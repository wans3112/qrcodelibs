...
////////////////////////////////////////////////////////////////////////////////////////////////////
- (id)initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled useEnglish:(bool) isEng {
...
- (void)drawRect:(CGRect)rect {
  ...
  if (displayedMessage == nil) {
      if (english) 
          self.displayedMessage = @"Place a QR code inside the viewfinder rectangle to scan it.";
      else 
          self.displayedMessage = @"请将二维码放置于白色的框内以便扫描。";
  }
  ...
  //	CGContextSetStrokeColor(c, white);
	...
	if (oneDMode) {
        char *text = nil;
        if (english) {
            text = "Place a red line over the QR code to be scanned.";
        } else {
            text = "请在二维码上方放置一根红线以方便扫描。";
		}
